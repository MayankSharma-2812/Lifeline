...
| **JavaScript — Promises vs callbacks** | [`client/src/components/EmergencyFormScreen.tsx`](file:///c:/Users/mayan/lifeline/client/src/components/EmergencyFormScreen.tsx) | Line 43 `navigator.geolocation.getCurrentPosition(successCb, errorCb)` error-first callback vs line 63 `pingIntakeTelemetry()` raw Promise `.then().catch()` chain vs line 90 `await createEmergencyRequestApi(...)` async-await Promise | Callback style lacks unified error bubbling; raw Promise `.then().catch()` allows non-blocking side-effects without async wrapper | Native callback APIs require separate error arguments; Promise chaining allows fluent transformations and unified error catching |
| **JavaScript — Event loop** | [`client/src/components/EmergencyFormScreen.tsx`](file:///c:/Users/mayan/lifeline/client/src/components/EmergencyFormScreen.tsx) | Line 106 `Promise.resolve().then(...)` microtask vs Line 111 `setTimeout(..., 0)` macrotask inside `handleSubmit` | If view transition were scheduled synchronously before microtasks drain, state logs/cleanup would execute out of order | Microtask queue guarantees immediate execution before browser repaint, while timer macrotasks execute in the next event loop turn |
